...
command -v lolcat >/dev/null 2>&1 || pip install lolcat >/dev/null 2>&1

figlet -f slant "LABBAIK" | lolcat
sleep 1
...
if command -v pkg >/dev/null 2>&1; then
    ...
    pkg install zip unzip -y >/dev/null 2>&1
    pkg install libcurl -y >/dev/null 2>&1
else
    # Debian/Ubuntu/Linux
    ...
    sudo apt install git curl wget unzip zip -y >/dev/null 2>&1
    sudo apt install figlet -y >/dev/null 2>&1
fi

# Install required Python modules
pip install --upgrade pip >/dev/null 2>&1
pip install requests tqdm pycurl lolcat rich bs4 colorama >/dev/null 2>&1

# Clone the Labbaik tool
clear
echo -e "\e[1;34m🚀 Cloning Labbaik Tool...\e[0m"
rm -rf Labbaik-Tool
git clone --depth=1 https://github.com/Labbaik757/Labbaik-Tool.git

cd Labbaik-Tool || {
    echo -e "\e[1;31m❌ Failed to enter tool directory.\e[0m"
    exit 1
}
...
if command -v xdg-open >/dev/null 2>&1; then
    ...
    termux-open-url "https://wa.me/923079741690"
else
    echo -e "\e[1;33m📱 Open manually:\e[0m https://wa.me/923079741690"
fi
